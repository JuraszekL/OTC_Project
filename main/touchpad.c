...
void TouchPad_Task(void *arg){
	...
	while(1){
		...
		// wait for notification from touch interrupt function
		...
		if(0 != notification_value){
			...
			if(FT6X36_GetTouchPointEXY(&data.evt, &data.x, &data.y) == FT6X36_OK){
				...
				// send the data to recieve queue
...
void TouchPad_PeriphInit(void){
	...
	// configure touchpad interupt gpio pin
...
// interrupt generated by touchpad pin
